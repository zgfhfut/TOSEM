...
int main()
{
	...
	for (o = 1; o < 31; o++)
	{
		...
		for (int_exp = 0; int_exp < MAX_EXPERIMENT; int_exp++)
		{
			...
			FILE* end_ptr;
			char file1[500];
			sprintf_s(file1, "D:\\论文代码数据\\experimental data\\single-input and single-output\\NSGA2-RR\\100\\Data of 30 instances\\test%d\\fitness%d.txt", o, int_exp + 1);;//在file1中写上每次迭代产生的不同fitness文件的路径
	...
	for (i = 0; i < popsize; i++)
	{
		...
		if (sum > time_stage)
		{
			...
			for (j = 0; j < nvar; j++)
			{
				...
				pop_ptr->ind[i].xreal[j] = pop_ptr->ind[i].xreal[j] * time_stage * rnd3 / sum;
			}
