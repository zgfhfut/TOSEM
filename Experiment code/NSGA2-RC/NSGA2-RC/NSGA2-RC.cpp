...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	for (o = 1; o < 31; o++)
	{
		...
		for (int_exp = 0; int_exp < MAX_EXPERIMENT; int_exp++)
		{
			...
			FILE* end_ptr;
			char file1[500];
			sprintf_s(file1, "D:\\论文代码数据\\experimental data\\single-input and single-output\\NSGA2-RC\\100\\Data of 30 instances\\test%d\\fitness%d.txt", o, int_exp + 1);;//在file1中写上每次迭代产生的不同fitness文件的路径
	...
	for (i = 0; i < popsize; i++)
	{
		...
		if (sum > time_stage)
		{
			...
			for (j = 0; j < nvar; j++)
			{
				...
				pop_ptr->ind[i].xreal[j] = (pop_ptr->ind[i].xreal[j]) * time_stage / sum * d;
			}
			for (j = 0; j < nvar; j++)
			{
				if (pop_ptr->ind[i].xreal[j] < lim_r[j][0]) pop_ptr->ind[i].xreal[j] = lim_r[j][0];
			}
		}
	...
	for (i = 0; i < popsize / 2; i++)
	{
		...
		if (rnd1 <= pcross)
		{
			...
			if (sum_chld1 > time_stage)/*chld1 violates the upper constraint,then it is certain than sum_chld2<yu2 based on the average property*/
			{
				...
				{
					q = pos_cross[k];
					if (new_pop_ptr->ind[n].xreal[q] < lim_r[q][0])new_pop_ptr->ind[n].xreal[q] = lim_r[q][0];
					if (new_pop_ptr->ind[n + 1].xreal[q] < lim_r[q][0])new_pop_ptr->ind[n + 1].xreal[q] = lim_r[q][0];
				}
