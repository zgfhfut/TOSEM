...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	for (o = 1; o < 31; o++)
	{
		...
		for (int_exp = 0; int_exp < MAX_EXPERIMENT; int_exp++)
		{
			...
			FILE* end_ptr;
			char file1[500];
			sprintf_s(file1, "D:\\论文代码数据\\experimental data\\single-input and single-output\\myNSGA2-RC\\100\\Data of 30 instances\\test%d\\fitness%d.txt", o, int_exp + 1);//在file1中写上每次迭代产生的不同fitness文件的路径
...
void realcross(population* new_pop_ptr, population* mate_pop_ptr)
{
	...
	int k, q;
	double rnd2, low_T, low_C;
	double sum_chld1, sum_chld2, sum_cross1, sum_cross2, temp1, temp2, temp3, temp4;
	double sum_c1, sum_c2, sum_cross_c1, sum_cross_c2;
	...
	for (i = 0; i < popsize / 2; i++)
	{
		...
		if (rnd1 <= pcross)
		{
			...
			if (sum_chld1 > time_stage)/*chld1 violates the upper constraint,then it is certain than sum_chld2<yu2 based on the average property*/
			{
				...
				if (sum_cross1 - time_stage + sum_chld2 + low_T <= 0)
					gama = 0;
				else {
					if (time_stage - sum_chld1 + sum_cross1 - low_T == 0)
						gama = 1.0;
					else
						gama = (sum_cross1 - time_stage + sum_chld2 + low_T) / (time_stage - sum_chld1 + sum_cross1 - low_T);
					if (gama > 1.0)
						gama = 1.0;	//if (time_stage - sum_chld1 + sum_cross1)==low_T							
				}
				rnd2 = randomperc();
				for (k = 0; k < no_cross; k++)//Only repair the mutation genes to maintain and inherit the parent's information
				{
				   ...
				   /*rnd2 = randomperc();*/
					new_pop_ptr->ind[n].xreal[q] = (temp1 - lim_r[q][0]) * (rnd2 * (1.0 - gama) + gama) * ((time_stage - sum_chld1 + sum_cross1 - low_T) / (sum_cross1 - low_T)) + lim_r[q][0];
			...
			if (sum_chld2 > time_stage)/*chld2 violates the upper constraint,then it is certain than sum_chld1<yu1 based on the average property*/
			{
				...
				if (sum_cross2 - time_stage + sum_chld1 + low_T <= 0)
					gama = 0;
				else {
					if (time_stage - sum_chld2 + sum_cross2 - low_T == 0)
						gama = 1.0;
					else
						gama = (sum_cross2 - time_stage + sum_chld1 + low_T) / (time_stage - sum_chld2 + sum_cross2 - low_T);

					if (gama > 1.0)
						gama = 1.0;// if (yu - sum_chld2 + sum_cross2)==low_T							
				}
				rnd2 = randomperc();
				for (k = 0; k < no_cross; k++)//Only repair the mutation genes to maintain and inherit the parent's information
				{
					...
					new_pop_ptr->ind[n + 1].xreal[q] = (temp1 - lim_r[q][0]) * (rnd2 * (1.0 - gama) + gama) * ((time_stage - sum_chld2 + sum_cross2 - low_T)/ (sum_cross2 - low_T)) + lim_r[q][0];
...
void real_mutate(population* new_pop_ptr)
{
	...
	for (j = 0; j < popsize; j++)
	{
		...
		if (sum > time_stage)
		{
			...
			{
				q = pos_muta[k];
				if (new_pop_ptr->ind[j].xreal[q] < lim_r[q][0])new_pop_ptr->ind[j].xreal[q] = lim_r[q][0];
			}
		}
