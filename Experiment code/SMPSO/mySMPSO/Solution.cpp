...
double Solution::cal_T(vector<double>& value)
{
    double sum_t = 0;
...
double Solution::cal_R(vector<double>& value)
{
    double rel, rel1;
    double r = 1;
    for (int i = 0; i <nreal; ++i)
    {
        rel = para_module[i][0] * para_module[i][1] * exp(-para_module[i][1] * value[i]);
        rel1 = exp(-rel * Mis_tim[i]);
        r = r * rel1;
    }
    return r;
}

double Solution::cal_C(vector<double>& value)
{
    double cost = 0;
    for (int i = 0; i < nreal; ++i)
        cost += (para_module[i][2] - para_module[i][3]) * para_module[i][0] * (1 - exp(-para_module[i][1] * value[i])) + para_module[i][3] * para_module[i][0] + para_module[i][4] * value[i];
    return cost;
}
 ...
 */
void Solution::setObjective(int i,vector<double>&value) {
  ...
  if(i==0)
  objective_[i] = 1-cal_R(value);
  else if(i==1)
  objective_[i] = cal_C(value);  
  else if (i == 2)
  objective_[i] = cal_T(value);
} // setObjective
