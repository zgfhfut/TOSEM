...
double SMPSO::repair_s(double low, double high, double l, double r)
{
...
void  SMPSO::repair_all(XReal* particle, double lim[], int count[])
{
...
void SMPSO::computeNewPositions() {
  for (int i = 0; i < swarmSize_; i++) {
    ...
    for (int var = 0; var < particle->getNumberOfDecisionVariables(); var++) {
         double Old1 =particle->getValue(var);
         particle->setValue(var, particle->getValue(var) +  speed_[i][var]) ;
         double Old2 = particle->getValue(var);
         count[var]=1;

      if (particle->getValue(var) < problem_->getLowerLimit(var))
      {
         
         double New = repair_s(problem_->getLowerLimit(var), problem_->getUpperLimit(var), Old1, Old2);
         particle->setValue(var, New);
      ...
      if (particle->getValue(var) > Old1)
          lim[var] = Old1;
      else lim[var] = problem_->getLowerLimit(var);
 

    }// for
    repair_all(particle, lim, count);
